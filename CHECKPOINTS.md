...
| Scope of the NPCI bands | **UPI Autopay only.** Cards do not route through that rail and emandate settles through NACH. `windows.py` said so in its docstring and the code applied the bands to every method, so the baseline was charged with violations on rails the rule does not govern — 705 became 213 on a mandate-heavy mix once scoped. The agent still applies them universally, which is conservative rather than correct; see issue #30 |
